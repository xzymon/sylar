...
# Zauważ że tu <typ-waloru>=crypto
...
# i - znając prefix - kopiuje do katalogu roboczego archiwum pliki spełniające maskę którą jest po prostu prefix.
...
# wszystkie pliki dla danego sposobu reprezentacji należące do danego miesiąca - podając jako prefix br_202502 - wyciągamy cały luty 2025.
...
if [ "$#" -lt 4 ]; then
    ...
    # Przykładowo:
    # $ bash $SYLAR_PROJECT_SCRIPTS_DIR/archive_pngs_crypto.bash stooq_png_crypto_archive_202510.zip 202510 br_202510 $SYLAR_DATA_SRC_IN_DIR/valors_crypto
    exit 1
fi

# Stałe - tu ustawiamy na sztywno crypto
...
prefix="$3"           # Prefiks plików - np. br_202502
...
while IFS= read -r directory; do
    ...
    if [ -d "$DIRS_PARENT_PATH/$directory" ]; then
        ...
        find "$DIRS_PARENT_PATH/$directory" -type f -name "${prefix}*" -exec cp -v -- "{}" "$work_dir/$directory/" \;
